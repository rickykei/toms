...
function Body($invoice_no)
{
 ...
 $this->SetY(-3);
  ...
  $border=0;
	...
	$this->Cell(5,8,"",$border,0,'R',0);
	//$this->Cell(5,8,""),$border,0,'R',0);
	$this->Cell(60,8,"姓名:".$customer_name,$border,0,'L',0);
	$this->Cell(65,8,"售貨員:".$sales_name,$border,0,'C',0);
	$this->Cell(75,8,"發票編號:".$branchid.$invoice_no,$border,1,'R',0);
...
//	$this->Cell(170,8,$customer_detail,0,1,'L',0);
	...
	$this->Cell(75,6,"貨品編號",1,0,'C',0);
	$this->Cell(65,6,"濶高",1,0,'C',0);
	$this->Cell(12,6,"數量",1,0,'C',0);
    $this->Cell(18,6,"單價",1,0,'R',0);
	$this->Cell(30,6,"總金",1,1,'R',0);
...
//print goods_invoice
	...
	while ($row2 =& $result->fetchRow(DB_FETCHMODE_ASSOC))
   {
	...
	$this->Cell(65,8,$row2['goods_partno']."碎料",$border,0,'L',0);
	
	//201712 display all mm / inch to ->mm
	if ($cal_unit=="mm"){
			$this->Cell(65,8, $row2['width'].' x '.$row2['height'].'['.$cal_unit_label.']',$border,0,'L',0);
	}else{
			$row2['width']=$row2['width']*25.416;
			$row2['height']=$row2['height']*25.416;
			$this->Cell(65,8, round($row2['width']).' x '.round($row2['height']).'[mm]',$border,0,'L',0);
	}
	
	$this->Cell(10,8,number_format($row2['qty']),$border,0,'R',0);
  
  	$this->Cell(18,8,number_format($row2['unit_price']),$border,0,'R',0);
	
	$amount=$row2['subtotal'];
	$this->Cell(30,8,number_format($row2['subtotal'], 2, '.', ','),$border,1,'R',0);
