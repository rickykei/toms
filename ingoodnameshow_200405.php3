...
echo "$date";
$openstring="/home/vhost/tomsracing.com.hk/invoice_store/";
$openstring.="$date";
$openstring.=".csv";
...
echo $handin_month;
echo $handin_day;
$search_date=$handin_year."-".$handin_month."-".$handin_day;
   ...
   else
   {
	...
	$openstring.="$date";
	$openstring.=".csv\">DOWNLOAD EXCEL FORMAT</a>";
	echo "$openstring";
	echo '</tr>',"\n";
	...
	for ($i=0;$i<count($rows);$i++)
         {
            ...
            {   $bgcolor='bgcolor="#0066cc"';
             ...
             $query="select sum(stock) from goods where goods_partno=\"$goods_partno\" and date < \"".$search_date." 23:59:59\"";
		$rows2=mysql_query($query);
             ...
             $query="select sum(qty) from goods_invoice,invoice where goods_invoice.invoice_no=invoice.invoice_no and goods_invoice.goods_partno = \"$goods_partno\" and invoice.invoice_date < \"".$search_date." 23:59:59\"";
       ...
       //     echo '<td ',$bgcolor,' ',$bgtext,'>',$goods_id,'</td>',"\n";
            ...
            echo '</form></td>',"\n";
            echo '</tr>',"\n";
	$string="$id".","."\"$goods_partno\"".","."\"$goods_detail\"".","."$market_price".","."$alllstock"."\n";
           fputs($fp,$string);
