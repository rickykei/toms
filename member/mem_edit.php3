...
<?
	...
	$id = genid();
	//mysql_close();
	$rows=mysql_query("select * from member where id=$ed");
$row=mysql_fetch_array($rows)
?>
<div align="center">
  <form name="memadd" method="POST" action="mem_ed.php3">
    <table width="620" border="0" cellpadding="2" cellspacing="2">
	  ...
	  <tr bgcolor="#7E94F8"> 
        <td colspan=" 2"><span class="style7">會員號碼：
          <input type="text" name="mem_id" size="10" maxlength="10" class="login" value=<?echo $row["mem_id"]?>>
        </span></td>
        <td colspan="2"><span class="style7">出生日期：
          <input type="text" name="dob" size="10" maxlength="10" class="login
" value=<?echo $row["mem_dob"]?>>
          &lt;dd-mm-yyyy&gt;</span></td>
      </tr>
      <tr bgcolor="#7E94F8"> 
...
&lt;yyyy&gt; </span></td>
      ...
      <tr bgcolor="#7E94F8"> 
        <td colspan="2"><span class="style7">入會日期 : 
          <input type="text" name="apply_date" size="10" maxlength="10" class="login" value=<?echo $row["apply_date"];?>>
        &lt;dd-mm-yyyy&gt;</span></td>
        <td colspan="2"><span class="style7">到期日期 : 
          <input type="text" name="exp_date" size="10" maxlength="10" class="login" value=<?echo $row["exp_date"];?>>
        &lt;dd-mm-yyyy&gt;</span></td>
      </tr>
      <tr bgcolor="#7E94F8"> 
