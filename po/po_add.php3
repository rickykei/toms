...
/* remove good_id by 10-7-02 
 ...
 //insert to po DB
   if ($customer=="y") 
      $query="insert into po (po_no,po_date,sales_name,customer_name,customer_tel,customer_detail,status) values ('$po_no',now(),'$sales_name','$customer_name','$customer_tel','$customer_detail','Y')";
   else if ($customer=="n")
  {
  // 16-11-01 add check memeber_detail from member_id
 $query="select mem_name_eng,mem_tel from member where mem_id='$member_id'";
 $result = mysql_query ($query);
 $row_mem_detail=mysql_fetch_array($result);
 $customer_name=$row_mem_detail["mem_name_eng"];
 $customer_tel=$row_mem_detail["mem_tel"];

 $query="insert into po (po_no,po_date,sales_name,customer_name,customer_tel,customer_detail,member_id,status) values ('$po_no',now(),'$sales_name','$cutomer_name','$customer_tel','$customer_detail','$member_id','Y')";
} 
 //if success to add then add goods_po DB
      
   if (!mysql_query($query))
    $cherr=3;
   else
   {
    for ($i=1;$i<$gitem+1;$i++) //insert goods_po
     {
     $query1="insert into goods_po (po_no,goods_id,goods_partno,qty,discountrate,marketprice,status) values ('$po_no','$goods_id[$i]','$goods_partno[$i]',$qty[$i],'$discountrate[$i]','$market_price[$i]','Y')";
...
<?
   ...
   if ($cherr==0)
   {
	...
	echo "<tr> ";
	echo "<td width=\"46%\" class=\"login\">發票編號.:".$po_no."</td>";
	echo "<td width=\"54%\" class=\"login\">發票日期: ".$po_date."</td>";
	echo "</tr>";
	echo "<tr> ";
	...
	echo "</td>";
	echo "<td width=\"54%\">會員編號: &nbsp;</td></tr><tr>";
	echo "<td width=\"46%\">&nbsp;</td>";
...
\nowidctlpar\intbl\aspalpha\aspnum\faauto\adjustright \kerning2\loch\af0\hich\af0\dbch\f15\cgrid {\cell \cell \cell \hich\af0\dbch\af15\loch\f0 C.O.D\cell \hich\af0\dbch\af15\loch\f0 PO-'.$po_no.'\cell }\pard 
...
\nowidctlpar\intbl\aspalpha\aspnum\faauto\adjustright {\hich\af0\dbch\f18\loch\af0 '.$customer_name.'\cell \cell \cell \hich\af0\dbch\af15\loch\f0 '.$po_date.'\cell }\pard \widctlpar\intbl\aspalpha\aspnum\faauto\adjustright {\row }\pard 
\nowidctlpar\aspalpha\aspnum\faauto\adjustright {
...
function printout()
{
window.open('./po_store/po-<?echo $po_no;?>.rtf');
window.location="po_ap.php3";
}
</script>
</body>
</html>
