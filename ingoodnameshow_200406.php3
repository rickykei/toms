	...
	for ($i=0;$i<count($rows);$i++)
         {
            ...
            {   $bgcolor='bgcolor="#0066cc"';
             ...
             $query="select sum(stock) from goods where goods_partno=\"$goods_partno\" and date <=\"$search_date\"";
echo $query;
		$rows2=mysql_query($query);
             ...
             $query="select sum(qty) from goods_invoice,invoice where goods_invoice.invoice_no=invoice.invoice_no and goods_invoice.goods_partno = \"$goods_partno\" and invoice.invoice_date<=\"$search_date\"";
